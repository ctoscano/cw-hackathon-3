...
export default function Aurora({
  ...
  useEffect(() => {
    ...
    const stopsArr = colorStops.map((hex) => {
    ...
    const update = (t: number) => {
      ...
      if (programRef.current) {
        programRef.current.uniforms.uTime.value = t * 0.01 * (p.speed ?? 1.0) * 0.1;
        programRef.current.uniforms.uAmplitude.value = p.amplitude ?? 1.0;
        programRef.current.uniforms.uBlend.value = p.blend ?? 0.5;
        programRef.current.uniforms.uColorStops.value = (p.colorStops ?? colorStops).map(
          (hex: string) => {
            const c = new Color(hex);
            return [c.r, c.g, c.b];
          },
