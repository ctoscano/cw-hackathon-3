...
```
TherapistSessionInput {
  scenario: string  // Brief clinical scenario description (1-2 sentences)
  therapeuticApproach: string  // Primary therapeutic modality used (e.g., CBT, DBT, psychodynamic, person-centered)
  patientPresentation: string  // How the patient presented: mood, affect, engagement level, notable behaviors
  keyThemes: string[]  // Main themes, topics, or issues discussed during the session
  interventions: string[]  // Specific therapeutic interventions employed during the session
  sessionNarrative: string  // Detailed first-person narrative of what the therapist would say when describing the session verbally. Should be conversational, as if speaking to a colleague.
}
```
