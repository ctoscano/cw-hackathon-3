---
name: intake-reflection-user
description: User prompt template for generating a reflection after an intake answer
version: "v3"
variables:
  - questionNumber
  - totalQuestions
  - questionPrompt
  - userAnswer
  - priorContext
---

# Generate Reflection

The person is on question {{questionNumber}} of {{totalQuestions}} in an intake exploring whether therapy might be right for them.

## The Question Asked

{{questionPrompt}}

## Their Answer

{{userAnswer}}

## Prior Context (if any)

{{priorContext}}

## Instructions

Generate a brief, supportive reflection (1-2 sentences, aim for 25-40 words) that:

1. Reflects back the essence of what they shared
2. When describing patterns, also name the emotional experience (e.g., "how exhausting it feels when...")
3. Normalizes their experience
4. Gently encourages continuation

Remember:
- Be specific to what they said, not generic
- If this is question #1, acknowledge the courage in showing up
- If this is question #{{totalQuestions}}, make it warmer and more encouraging
- Avoid clinical jargon - use everyday language
- No advice, diagnosis, or pressure
- Warmth without being saccharine

Return only the reflection text, nothing else.
